...
public class Track implements Cloneable {
	...
	public void draw (Graphics g, int X, int Y, int aTileOrient, Hex aHex, Color aBaseColor, Feature2 aSelectedFeature) {
		Location tEnter;
		Location tExit;
		Area tNewClip;
		Area tHexClip;
	...
	public void drawTrackEdgeToEdge (Graphics g, int X, int Y, Location aEnter, Location aExit, Hex aHex, Color aBaseColor) {
	...
	public void drawTrackToCity (Graphics g, int X, int Y, int aTileOrient, Hex aHex, Color aBaseColor) {
		...
		if (tExit.isCenterLocation ()) {
			...
			if (tExit.isClose (tEnter)) {
				...
				// If neither the Enter or Exit are sides, (city to city), draw straight segment between
				...
				if (tExit.isDeadEnd ()) {
					...
					// If nothing else above matches, draw just a straight Track Segment between Locations.
					displace = tEnter.calcCenter (aHex);
					tStartX = X + displace.x;
					tStartY = Y + displace.y;
					displace = tExit.calcCenter (aHex);
	...
	public BasicStroke getTrackStroke (Hex aHex) {
		...
		if (isDashed) {
			tTrackStroke = new BasicStroke (tTrackWidth, BasicStroke.CAP_BUTT, 
