	...
	private void createMap () {
		...
		if (gameIsStarted ()) {
			...
			try {
				...
				tMapFrame.loadXML (tXMLDocument, tHexMap);
			} catch (Exception tException) {
				logger.error (tException);
...
//		}
	...
	protected void addAllElements (XMLDocument tXMLDocument, XMLElement tSaveGameElement) {
...
//        		}
	...
	public String addChecksum (ElementName aEN_Name, XMLDocument aXMLDocument) {
		...
		tActionNumber = roundManager.getActionNumber ();
	...
	public void fillChecksumAuditFrame () {
		...
		if (checksums != Checksums.NO_CHECKSUMS) { 
			...
			for (tChecksumIndex = 0; tChecksumIndex < tChecksumCount; tChecksumIndex++) {
				tChecksum = checksums.get (tChecksumIndex);
				checksumAuditFrame.addRow (tChecksum, true);
			}
		}
	}
